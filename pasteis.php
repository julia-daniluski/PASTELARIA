...
if (isset($_POST['add'])) {
    ...
    if (empty($erros)) {
        ...
        $stmt = $conn->prepare("INSERT INTO pasteis (nome, ingredientes, preco, tamanho, categoria, estoque_minimo, ativo) VALUES (?, ?, ?, ?, ?, ?, 1)");
        if ($stmt) {
            $stmt->bind_param('ssdssi', $nome_pastel, $ingredientes_pastel, $preco, $tamanho_pastel, $categoria_pastel, $estoque_min);
            if ($stmt->execute()) {
                $_SESSION['flash_success'] = 'pastel cadastrada com sucesso.';
...
<head>
    ...
    <title>Gerenciar pasteis - Sistema pastelria</title>
